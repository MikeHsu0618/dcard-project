...
同時可以利用快取資料庫的特點實現分佈鎖以及前兩年 Dcard 作業中的主題 `IP 限制器` 來解決高併發以及請求限制的需求
...
但雜湊函數無法避免「雜湊碰撞（collision）」，因此有一定機率會出現多個長網址對應到同一個縮網址，
...
一個 loop 打爆你的服務嗎？ 
   ...
   所以本專案祭出幾個解決思路：
   - 不要從 1 開始發號, 沒錯聽起來很簡單, 當我們服務初期發號量小的時候確實很容易猜出發號策略是連續的,
  ...
  即可獲得 `原網址 index`, 此舉降低了 `初期外人看出發號策略的機率`
   - 而連續發號的問題可以利用, Postgres `sequences` 的特性為當他發出了自增的 index 值後, 不管該事務有沒有 rollback 都不會消耗額外性能對事務做第二次檢查,
   都會繼續遞增下去, 假設五次 `insert` 請求中有三次失敗 (1, 2(失敗), 3(失敗), 4(失敗), 5), 
   則資料庫此時將有兩筆資料, index id 分別為 `1, 5`, 減緩了 `發號數純粹連續` 的問題
   - 藉由我們對`原網址`欄位下的 Unique 索引, 只要使用者只要輸入了`已存在資料庫的原網址`,
   即可天然的產生一次 `insert 失敗`, 產生 index 無規則不連續跳號並取得原有資料,
   此舉處理了 `使用者重複產生相同網址的問題` 以及 `給予合理可預期的天然操作失敗已達成跳號目的`
   - 多次請求的問題採用 Redis 實現的 `IP 限制中介`, 對每個 IP 進行一小時內最多只能傳送 100 次請求的限制,
   達成對請求過多的問題
...
5. 後端實現：使用 Gin 框架運行 Server , 並加入 `PostgreSQL 資料庫` `Redis IP 限制器中介` `Redis 分佈式鎖` `Redis 快取`
   來完成作業兩支指定 APIs
     - `Post  "/" create short_url` 收到前端的請求後新增的`原網址` -> call 一次該網址確認是否為合法網址,
     ...
     - `Get "/link/:shortUrl" redirect to org_url` 當收到前端請求後, 將 62進制的短網址轉為 10 進制的 index id 並減去預設基數, 
     ...
     而不是理想的, 第 1 筆資料 Redis 查詢無果後轉而查詢資料庫並建立快取, 第 2 ~ 1000 筆請求皆可以使用
     到第 1 筆請求建立的快取, 成功取得 `原網址` 則直接利用 `302 redirect` 轉導前端到該網頁
